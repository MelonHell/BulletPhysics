...
class NmsTools_v1_19_2 : NmsTools {
    ...
    override fun getHeight(fluidState: FluidStateWrapper, block: Block): Float {
        return NmsWrapUtils_v1_19_2.unwrap(fluidState).getHeight((block as CraftBlock).handle, block.position)
    }

    override fun getFlow(fluidState: FluidStateWrapper, block: Block): Vector {
        val flow: Vec3 = NmsWrapUtils_v1_19_2.unwrap(fluidState).getFlow((block as CraftBlock).handle, block.position)
    ...
    override fun boundingBox(block: Block, blockState: BlockState): BoundingBox? {
        ...
        return if (voxelShape.isEmpty) null else NmsWrapUtils_v1_19_2.convert(
            voxelShape.bounds()
        )
    }
    ...
    override fun boundingBoxes(block: Block, blockState: BlockState): List<BoundingBox> {
        ...
        return boundingBoxes.map { obj: AABB -> NmsWrapUtils_v1_19_2.convert(obj) }
    }
