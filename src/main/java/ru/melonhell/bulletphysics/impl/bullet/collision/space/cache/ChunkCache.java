...
public interface ChunkCache {
    ...
    static boolean isValidBlock(BlockState blockState) {
        ...
        return NmsUtils.getNmsTools().collidableCheck(blockState);
    }
...
//    }
    ...
    class FluidColumn {
        ...
        public FluidColumn(Block start) {
            ...
            var fluidState = NmsUtils.getNmsTools().getFluidState(cursor);

            // find bottom block
            while (!fluidState.isEmpty()) {
                cursor = cursor.getRelative(BlockFace.DOWN);
                fluidState = NmsUtils.getNmsTools().getFluidState(cursor);
            }

            cursor = cursor.getRelative(BlockFace.UP); // the above loop ends at one below the bottom
            fluidState = NmsUtils.getNmsTools().getFluidState(cursor);
            this.bottom = new FluidData(cursor, NmsUtils.getNmsTools().getFluidState(cursor));
            ...
            this.height = NmsUtils.getNmsTools().getHeight(fluidState, cursor);

            // Water flow direction
            this.flow = Convert.toBullet(NmsUtils.getNmsTools().getFlow(fluidState, cursor));
        }
